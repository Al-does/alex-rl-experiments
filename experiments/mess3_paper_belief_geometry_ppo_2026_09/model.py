"""Paper-scale pre-LN causal transformer and its PPO actor-critic variant."""
...
class PaperActorCritic(PaperTransformer):
    """Paper transformer whose unembedding is the policy; adds a value head."""

    def __init__(self, config: PaperModelConfig | None = None):
        super().__init__(config)
        self.value_head = nn.Linear(self.config.d_model, 1, bias=True)
        self._initialize(self.value_head)

    def policy_and_value(
        self,
        tokens: torch.Tensor,
    ) -> tuple[torch.Tensor, torch.Tensor]:
        logits, activations = self(tokens, return_activations=True)
        values = self.value_head(activations["final_ln"]).squeeze(-1)
