"""Correctness reward for the pending RRXOR edge token."""
...
class RRXORTokenGuessTask:
    """Reward a categorical guess that matches the pending hidden token."""

    requires_belief = False

    def __init__(self, *, model: HMMModel) -> None:
        if model.edge_transition_matrices is None:
            raise ValueError("RRXOR token guessing requires edge emissions")
        self._model = model
    ...
    def resolve_action(
        ...
        if model is not self._model:
            raise ValueError("task must use the model it was constructed with")
        if (
    ...
    def encode_action(self, executed_action: int) -> np.ndarray:
        if (
            isinstance(executed_action, (bool, np.bool_))
            or not isinstance(executed_action, (int, np.integer))
            or not self.action_space.contains(executed_action)
        ):
