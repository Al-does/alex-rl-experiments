...
from experiments.strata_two_factor_explore_cycle_3.context_length_64_entropy_4x.process import (
    ACTION_PAIRS,
    CONTEXT_LENGTH,
    EPISODE_LENGTH,
    FACTOR_COUNT,
...
_STRATA_EMISSION_MATRIX = strata_model(
    alpha=STRATA_ALPHA,
    t0=STRATA_T0,
    t1=STRATA_T1,
).emission_matrix
_, _, _STRATA_EMISSION_VECTORS = np.linalg.svd(
    _STRATA_EMISSION_MATRIX.T,
    full_matrices=True,
)
_STRATA_NULL_DIRECTION = _STRATA_EMISSION_VECTORS[-1]
if _STRATA_NULL_DIRECTION[0] < 0:
    _STRATA_NULL_DIRECTION *= -1
...
def _factor_report(predicted: np.ndarray, target: np.ndarray) -> dict[str, Any]:
    return {
        **_metrics(predicted, target),
        "delta": _metrics(
            (predicted @ _STRATA_NULL_DIRECTION)[:, None],
            (target @ _STRATA_NULL_DIRECTION)[:, None],
        ),
    }
...
def _analyze_samples(
    ...
    *,
    ...
    if emission_matrix.shape != (3, 2):
        raise ValueError("Strata next-token projection requires a (3, 2) emission matrix")
    ...
    result = {
        ...
        "metadata": {
            ...
            "delta_definition": (
                "unit null direction of the Strata emission matrix"
            ),
            "delta_direction": _STRATA_NULL_DIRECTION.tolist(),
            "delta_decoder": "contrast_of_affine_factor_belief_prediction",
            "normalized_mse_definition": "mse/test_target_variance",
            "ntp_definition": "factor_belief @ strata_model.emission_matrix",
...
def analyze_checkpoint(
    ...
    *,
    ...
    return _analyze_samples(
        ...
        streams=streams,
        emission_matrix=_STRATA_EMISSION_MATRIX,
    )
