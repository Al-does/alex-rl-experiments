...
from envs.strata.model import controlled_kernels
...
def emission_null_direction(kernels: np.ndarray) -> np.ndarray:
    emission_matrix = kernels[0].sum(axis=-1).T
    _, _, vectors = np.linalg.svd(emission_matrix.T, full_matrices=True)
    direction = vectors[-1]
    if direction[0] < 0:
        direction *= -1
    return direction
...
def identifiability_summary(kernels: np.ndarray) -> dict[str, Any]:
    delta = emission_null_direction(kernels)
    ...
    return {
        ...
        "interpretation": (
            "One-step NTP does not identify the reported Strata belief "
            "contrast. This null is not generally an invariant unobservable "
            "subspace of the edge dynamics: two-step token distributions can "
            "distinguish it, and arrival rewards can be sensitive to it."
        ),
    }
