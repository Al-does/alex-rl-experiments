"""Continue the completed Shot A variant 3 run from its final checkpoint.

The source run stopped at 15M sampled environment steps. This leaf restores
the final public Algorithm checkpoint into a freshly built Shot A variant 3
config (same schedules; the restored lifetime step count keeps the learning
rate and entropy schedules on their terminal segments), then trains until 30M
lifetime steps, saving a public checkpoint at every 3M-step boundary and
probing each saved checkpoint with the study's standard battery.
"""
...
VARIANT = 3
CONDITION = "shot_a_variant_3_continuation"
SOURCE_RUN_ID = "20260913T215641Z-227e63ee"
SOURCE_RESULT_COMMIT = "1ffd1a9fa3b871b7a9e1c522b2ddd290a4d17a89"
SOURCE_B2_PREFIX = (
    "experiments/nonergodic_mess3_reward_state_action_symmetry_cycle_5/"
    f"shot_a/variant_3/{SOURCE_RUN_ID}"
)
SOURCE_CHECKPOINT_KEY = (
    f"{SOURCE_B2_PREFIX}/tune/"
    "PPO_HMMEnv_0287c_00000_0_2026-09-13_21-56-44/checkpoint_000000"
)
SOURCE_AGENT_STEPS = 15_262_352
SOURCE_TRAINING_ITERATION = 58
TOTAL_ENV_STEPS = 30_000_000
CHECKPOINT_EVERY_STEPS = 3_000_000
...
def build_config(context: RunContext, checkpoint_root: Path):
    smoke = context.smoke
    return build_shot_a_config(context, VARIANT).callbacks(
