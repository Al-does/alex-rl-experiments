"""Continue the Shot A variant 3 continuation from its final checkpoint.

The source run stopped at ~30.25M sampled environment steps. This leaf
restores the final public Algorithm checkpoint into a freshly built Shot A
variant 3 config (the restored lifetime step count keeps the learning-rate
schedule on its terminal 1e-5 segment) with one override: the entropy
coefficient ramps linearly from 0 back to 0.01 across 30M-35M lifetime steps
...
SOURCE_RUN_ID = "20260914T042419Z-d63e24cc"
SOURCE_RESULT_COMMIT = "4b2410257df3a3ac1ce20abebb3eb8322c6bb8d3"
SOURCE_B2_PREFIX = (
    "experiments/nonergodic_mess3_reward_state_action_symmetry_cycle_5/"
    f"shot_a/variant_3_continuation/{SOURCE_RUN_ID}"
)
SOURCE_CHECKPOINT_KEY = (
    f"{SOURCE_B2_PREFIX}/checkpoints/iteration_000057_final"
)
SOURCE_AGENT_STEPS = 30_247_082
SOURCE_TRAINING_ITERATION = 115
    ...
    recipe.update(
        {
            ...
            "entropy_schedule_note": (
                "restored lifetime step count starts past 30M, so the "
                "coefficient ramps 0 -> 0.01 across 30M-35M lifetime steps "
