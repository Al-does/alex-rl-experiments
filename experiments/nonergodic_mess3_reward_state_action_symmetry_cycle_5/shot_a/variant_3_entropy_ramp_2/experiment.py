"""Continue the interrupted Shot A variant 3 entropy-ramp run from 195.24M.

The first entropy-ramp continuation was interrupted at ~195M+ lifetime steps
when its host rebooted; this leaf restores its 195.24M-step boundary
checkpoint into a freshly built Shot A variant 3 config (the restored
lifetime step count keeps the learning-rate schedule on its terminal 1e-5
segment) with the same entropy schedule: the coefficient ramps linearly from
0 to 0.01 across 30M-35M lifetime steps and holds 0.01 afterwards. Training
runs to the same 330.25M lifetime target, saving a public checkpoint at
every 15M-step boundary and probing each saved checkpoint with the study's
standard battery.
"""
...
VARIANT = 3
CONDITION = "shot_a_variant_3_entropy_ramp_2"
SOURCE_RUN_ID = "20260920T072355Z-699d04f0"
SOURCE_RESULT_COMMIT = "8c623c2106372113292693459f9623b802e4d6a0"
SOURCE_B2_PREFIX = (
    "experiments/nonergodic_mess3_reward_state_action_symmetry_cycle_5/"
    f"shot_a/variant_3_entropy_ramp/{SOURCE_RUN_ID}"
)
SOURCE_CHECKPOINT_KEY = (
    f"{SOURCE_B2_PREFIX}/checkpoints/iteration_000742_steps_195238370"
)
SOURCE_AGENT_STEPS = 195_238_370
SOURCE_TRAINING_ITERATION = 742
TOTAL_ENV_STEPS = 330_247_082
    ...
    recipe.update(
        {
            "condition": CONDITION,
            "continuation_of": {
                "condition": "shot_a_variant_3_entropy_ramp",
            ...
            "restore": (
                "fresh Shot A variant 3 config built on this box, then "
            ...
            "entropy_schedule_note": (
                "restored lifetime step count resumes at 195.24M, past the "
                "30M-35M ramp, so the coefficient holds 0.01 for the "
            ...
            "note": (
                "source run 20260920T072355Z-699d04f0 was interrupted past "
                "195M lifetime steps when its host rebooted; this leaf "
                "resumes from its 195.24M-step boundary checkpoint"
            ),
