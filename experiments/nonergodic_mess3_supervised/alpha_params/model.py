"""Paper-scale decoder-only transformer with exposed residual streams."""
...
from .data import CONTEXT_LENGTH, VOCAB_SIZE
...
class NonergodicMess3Transformer(nn.Module):
    """TransformerLens-style architecture used in the Simplex blog."""
    ...
    def residual_names(self) -> tuple[str, ...]:
        return ("embedding",) + tuple(
            f"block_{index}" for index in range(1, self.config.n_layers + 1)
        )
    ...
    def forward(
        ...
        *,
        ...
        residuals = {"embedding": hidden} if return_residuals else None
        for index, block in enumerate(self.blocks, start=1):
            hidden = block(hidden)
            if residuals is not None:
                residuals[f"block_{index}"] = hidden
