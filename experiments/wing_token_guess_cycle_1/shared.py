...
def resolved_recipe(context: RunContext) -> dict[str, Any]:
    return {
        "study": "wing_token_guess_cycle_1",
        "condition": "ppo",
        "seed": context.seed,
        "smoke": context.smoke,
        "hypothesis": (
            "PPO can learn the finite-context predictor for the pending "
            "two-factor Wing token from delayed token history."
        ),
        "primary_comparison": (
        ...
        "action_semantics": (
            "four categorical action logits, one per joint Wing token"
        ),
        "reward": (
        ...
        "context_semantics": (
            "strict last 32 delayed joint-token frames including current"
        ),
        "total_env_steps": (
        ...
        "analysis": (
            "held-out layerwise affine probes of exact delayed Wing beliefs"
        ),
        "intended_hardware": (
...
def run_condition(context: RunContext) -> dict[str, Any]:
    from experiments.wing_token_guess_cycle_1.analysis import analyze_checkpoint
