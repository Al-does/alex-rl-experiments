...
class WingTokenGuessTask:
    """Reward the action whose index matches the pending Wing token."""

    requires_belief = False

    def __init__(self, *, model: HMMModel) -> None:
        if model.edge_transition_matrices is None:
            raise ValueError("Wing token guessing requires an edge-emitting model")
        self._model = model
