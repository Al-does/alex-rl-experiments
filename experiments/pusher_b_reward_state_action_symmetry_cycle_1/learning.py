...
def next_token_targets(
    ...
    The delayed observation at step t+1 contains the token emitted by the
    transition decided at step t, so aux logits at position t supervise on the
    token one-hot slice of the following observation row. The token one-hot
    occupies the first ``num_classes`` observation columns; the appended
    executed-action one-hot is ignored.
    """
