"""Held-out belief probes and Kelly-specific policy diagnostics."""
...
def _policy_wagers(
    ...
    *,
    ...
    token_logits = module.action_distribution_inputs(embeddings)
    ...
    if condition == "fixed_full":
        wager = torch.full_like(selected_probability, MAX_WAGER)
    elif condition == "policy_implied_kelly":
        wager = kelly_fraction(selected_probability)
    elif condition == "learned_kelly":
        wager = module.wager_fraction(embeddings).clamp(max=MAX_WAGER)
    elif condition == "bayes_oracle":
        wager = torch.zeros_like(selected_probability)
    else:
...
def probe_checkpoint(
    ...
    *,
    ...
    """Probe beliefs and evaluate wager quality on independent greedy rollouts."""

    if context.seed is None:
        raise ValueError("Kelly probing requires a resolved seed")
    ...
    with load_algorithm(checkpoint) as algorithm:
        ...
        policy_probability, wager = _policy_wagers(
            module,
            test.activations,
            actions,
            condition=condition,
    ...
    exact_token_probabilities = test.beliefs @ emission_matrix
    exact_selected_probability = exact_token_probabilities[
        np.arange(len(actions)),
        actions,
    ]
    oracle_wager = kelly_fraction(exact_selected_probability)
    ...
    best_probability = exact_token_probabilities.max(axis=-1)
    ...
    metrics = {
        ...
        "wager_vs_oracle_rmse": float(
            np.sqrt(np.square(wager - oracle_wager).mean())
        ),
        ...
        "oracle_policy_ceiling_log_growth_mean": ceiling_mean,
    ...
    _plot_wager_calibration(
        oracle_wager[indices],
        wager[indices],
        condition=condition,
        path=context.results_dir / "wager_calibration.png",
    )
    return KellyProbeResult(metrics, target_display, decoded_display)
