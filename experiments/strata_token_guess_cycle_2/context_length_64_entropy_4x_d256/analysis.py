...
from experiments.strata_token_guess_cycle_2.context_length_64_entropy_4x_d256.process import (
    CONTEXT_LENGTH,
    EPISODE_LENGTH,
    FACTOR_COUNT,
    STRATA_ALPHA,
...
def _target_adapter(
    ...
    return {
        ...
        "factor_belief": np.stack(
            factor_marginals(joint, (3,) * FACTOR_COUNT),
            axis=1,
        ),
        "state": np.asarray(
...
def _observation_groups(observations: np.ndarray) -> np.ndarray:
    ...
    if observations.ndim != 2 or observations.shape[1] != TOKEN_COUNT:
        raise ValueError("expected one delayed Strata-token one-hot")
    return np.where(
...
def _policy_report(data: ProbeData) -> dict[str, Any]:
    ...
    return {
        ...
        "guess_fractions": (
            np.bincount(data.actions, minlength=TOKEN_COUNT) / count
        ).tolist(),
        "hidden_token_fractions": (
            np.bincount(data.hidden_tokens, minlength=TOKEN_COUNT) / count
        ).tolist(),
    }
    ...
    *,
    ...
    if consistency > 1e-10:
        raise AssertionError(
            f"single-factor belief reconstruction failed: {consistency:.3e}"
        )
    ...
    result = {
        ...
        "cev": {
            ...
            "state_belief_target": variance_geometry(test.joint_beliefs),
            "strata_belief_target": variance_geometry(test.factor_beliefs[:, 0]),
        },
