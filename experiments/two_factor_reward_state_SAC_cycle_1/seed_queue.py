"""Run one two-factor SAC arm across several seeds on a single GPU box.
...
Example (on a provisioned box, already inside the activated ``.venv``):
  ...
  python -m experiments.two_factor_reward_state_SAC_cycle_1.seed_queue \\
...
from experiments.two_factor_reward_state_SAC_cycle_1.task import CONDITIONS
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Sequential multi-seed runner for one "
            "two_factor_reward_state_SAC_cycle_1 arm."
        )
    )
