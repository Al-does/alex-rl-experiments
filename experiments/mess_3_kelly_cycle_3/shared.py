"""Four-arm gamma-.99 PPO/IQN comparison."""
...
class Arm:
    name: str
    critic_mode: str
    conditional_kelly: bool


ARMS = (
    Arm("ppo", "mean", False),
    Arm("iqn", "iqn", False),
    Arm("conditional_decoupled_kelly_mean", "mean", True),
    Arm("conditional_decoupled_kelly_iqn", "iqn", True),
)
...
def arm_by_name(name: str) -> Arm:
    try:
        ...
        raise ValueError(f"unknown cycle-3 arm {name!r}") from error


def _module_class(arm: Arm):
    if arm.conditional_kelly:
        return (
            IQNConditionalWagerTransformerModel
...
def _learner_config(arm: Arm) -> dict[str, Any]:
    ...
    if arm.conditional_kelly:
        config.update(
            {
                ACTOR_MODE_KEY: CONDITIONAL_MODE,
                WAGER_LAYOUT_KEY: CONDITIONAL_LAYOUT,
...
def build_config(context: RunContext, arm_name: str) -> PPOConfig:
    """Build one fresh gamma-.99 comparison arm."""

    if context.seed is None:
        raise ValueError("cycle 3 requires a resolved seed")
...
def run_condition(context: RunContext, arm_name: str):
    """Train and probe one arm without retaining verbose Tune summaries."""
    ...
    outputs.write_json(
        "resolved_recipe.json",
        {
            ...
            "direct_kelly_loss_weight": (
                DIRECT_LOSS_WEIGHT if arm.conditional_kelly else 0.0
            ),
