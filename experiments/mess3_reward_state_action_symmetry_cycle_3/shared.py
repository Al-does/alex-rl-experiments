"""Shared PPO recipes with optional Kelly / next-token CE auxiliaries."""
...
from experiments.mess3_reward_state_action_symmetry_cycle_3.analysis import (
    ProbeResult,
    plot_probe,
    probe_checkpoint,
)
...
Objective = Literal["decoupled_kelly", "predictive_loss"]


class PredictiveModel(NextTokenAuxHead, TransformerModel):
    """Transformer actor-critic with a training-only next-token head."""


class PredictiveLearner(NextTokenAuxLossMixin, PPOTorchLearner):
    """PPO plus next-visible-token cross entropy."""


class KellyModel(DecoupledKellyHead, TransformerModel):
    """Transformer actor-critic with a separate action-conditional Kelly head."""
...
def condition_name(variant: int, objective: Objective) -> str:
    """Return the canonical arm name used in results folders."""

    return f"variant_{variant}_{objective}"
...
def environment_config(variant: int) -> dict[str, Any]:
    ...
    return {
        ...
        "task": {
            "class": (
                "experiments.mess3_reward_state_action_symmetry_cycle_3.task:"
                "ActionSymmetryTask"
            ),
            "kwargs": {
                "variant": variant,
...
def _objective_wiring(objective: Objective) -> tuple[type, dict, type, dict]:
    """Return module class, model config, learner class, learner config."""

    if objective == "predictive_loss":
        return (
            PredictiveModel,
            {**BASE_MODEL_CONFIG, "next_token_aux": {"num_classes": 3}},
            PredictiveLearner,
            {
    ...
    if objective == "decoupled_kelly":
        return (
            ...
            KellyPPOTorchLearner,
            {KELLY_LOSS_COEFFICIENT_KEY: DIRECT_KELLY_LOSS_WEIGHT},
        )
...
def build_config(
    ...
    *,
    ...
    """Build one transformer PPO configuration with the requested auxiliary."""
    ...
    module_class, model_config, learner_class, learner_config = _objective_wiring(
        objective
    )
    config = (
        PPOConfig()
...
def run_condition(
    ...
    *,
    ...
    """Train one arm and probe init plus log-spaced checkpoints."""

    if context.seed is None:
        raise ValueError("action-symmetry cycle requires a resolved seed")
    condition = condition_name(variant, objective)
    ...
    _, model_config, _, learner_config = _objective_wiring(objective)
    recipe = {
        "condition": condition,
        "variant": variant,
        "objective": objective,
        ...
        "learner_config": {
            ...
            if key != "next_token_aux/target_extractor"
        },
