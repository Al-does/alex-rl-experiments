...
def factor_specifications() -> list[dict[str, Any]]:
    ...
    factory = (
        "experiments.two_factor_reward_state_PPO_cycle_1.process:"
...
def environment_config(condition: str) -> dict[str, Any]:
    """Build the action-aware recurrent PPO environment for one reward arm."""
    ...
    return {
        ...
        "task": {
            "class": (
                "experiments.two_factor_reward_state_PPO_cycle_1.task:"
                "TwoFactorShiftTask"
            ),
            "kwargs": {"condition": condition},
        },
        # Each recurrent frame aligns the visible joint token with the action
        # that generated it. The transformer carries the preceding 63 frames.
        "observation": {
            "token": {"depth": 1},
            "action": {"depth": 1},
        },
