...
from experiments.wing_two_factor_explore_cycle_2 import analysis, shared
...
def test_recipe_changes_only_control_value_cap_and_reward_placement(context, condition):
    leaf = importlib.import_module(
        f"experiments.wing_two_factor_explore_cycle_2.{condition}_state_0.experiment"
    )
    ...
    assert report["study"] == "wing_two_factor_explore_cycle_2"
...
def test_rotations_preserve_emission_map_and_exact_transducer_filter():
    ...
    try:
        ...
        base = wing_model().edge_transition_matrices
...
def test_checkpoint_probes_use_cycle_2_dynamics(context, monkeypatch):
    ...
    env.close()
    monkeypatch.setattr(analysis.probe, "load_algorithm", lambda _: nullcontext(SimpleNamespace(get_module=lambda: module)))
    seen = []
    original = analysis.probe.HMMEnv

    def collect_environment(config):
        seen.append(config["task"]["kwargs"]["strength"])
        return original(config)

    monkeypatch.setattr(analysis.probe, "HMMEnv", collect_environment)
