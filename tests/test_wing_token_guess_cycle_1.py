...
from experiments.wing_token_guess_cycle_1 import analysis
from experiments.wing_token_guess_cycle_1.process import (
    CONTEXT_LENGTH,
    JOINT_TOKEN_COUNT,
    environment_config,
)
from experiments.wing_token_guess_cycle_1.shared import (
    MODEL_CONFIG,
    SMOKE_BATCH_SIZE,
    SMOKE_ENV_STEPS,
...
from experiments.wing_token_guess_cycle_1.task import WingTokenGuessTask
...
def _module() -> WingActorCritic:
    return WingActorCritic(
        observation_space=gym.spaces.Box(
            0.0,
            1.0,
            shape=(JOINT_TOKEN_COUNT,),
            dtype=np.float32,
        ),
        action_space=gym.spaces.Discrete(JOINT_TOKEN_COUNT),
        model_config=dict(MODEL_CONFIG),
    ).eval()


def test_delay_one_action_logits_guess_pending_joint_token():
    ...
    try:
        ...
        assert env.action_space == gym.spaces.Discrete(JOINT_TOKEN_COUNT)
        assert isinstance(env.task, WingTokenGuessTask)
        np.testing.assert_array_equal(
            observation,
            np.zeros(JOINT_TOKEN_COUNT),
        )
        ...
        np.testing.assert_array_equal(
            next_observation,
            np.eye(JOINT_TOKEN_COUNT, dtype=np.float32)[pending_token],
        )
    finally:
        env.close()
...
def test_previous_reward_is_not_in_observation_or_belief():
    ...
    try:
        ...
        assert normal_observation.shape == (JOINT_TOKEN_COUNT,)
...
def test_model_emits_one_logit_per_joint_token():
    ...
    batch = {
        Columns.OBS: torch.zeros((1, 2, JOINT_TOKEN_COUNT)),
...
def test_probe_collection_uses_reward_aligned_targets_without_reward_inputs():
    ...
    assert data.factor_beliefs.shape == (64, 2, 3)
    assert data.observations.shape == (64, JOINT_TOKEN_COUNT)
...
@pytest.mark.parametrize("action", [-1, 4, 1.5, True, "1"])
