...
from experiments.cassandra_belief_factoring_2026_08.previous_reward_uniform_250m.fully_observable_global_alias_d64_3layer_4head.experiment import (
    build_config as build_fully_observable_global_alias_d64,
)
from experiments.cassandra_belief_factoring_2026_08.previous_reward_uniform_250m.fully_observable_targeted_d64_3layer_4head.experiment import (
    build_config as build_fully_observable_targeted_d64,
)
...
@pytest.mark.parametrize(
    ...
    [
        (build_fully_observable_targeted_d64, "targeted"),
        (build_fully_observable_global_alias_d64, GLOBAL_ALIAS_ACTION_SCOPE),
    ],
)
def test_fully_observable_recipes_use_state_and_match_baseline_hparams(
    smoke_context,
    builder,
    action_scope,
):
    baseline = build_d64(smoke_context)
    config = builder(smoke_context)
    ...
    assert config.vf_loss_coeff == baseline.vf_loss_coeff
    assert config.entropy_coeff == baseline.entropy_coeff
