...
from experiments.strata_two_factor_explore_cycle_3 import analysis, shared
...
def test_recipe_preserves_cycle_3_training_choices(context, condition):
    leaf = importlib.import_module(
        f"experiments.strata_two_factor_explore_cycle_3.{condition}_state_0.experiment"
    )
    ...
    assert config.env_config["task"]["kwargs"] == {
        "reward_state": 0,
    ...
    assert report["study"] == "strata_two_factor_explore_cycle_3"
...
def test_checkpoint_probes_use_cycle_3_dynamics(context, monkeypatch):
